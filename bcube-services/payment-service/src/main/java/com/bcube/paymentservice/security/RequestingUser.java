...
/**
 * The JWT's subject is the user's email (see user-service's JwtTokenProvider), but every
 * resource here (Payment.userId, booking-service's Booking.userId, ...) is keyed by the numeric
 * id, so the token also carries a "userId" claim specifically so services like this one can
 * check "does this resource belong to the caller" without an extra round-trip to user-service.
 */
